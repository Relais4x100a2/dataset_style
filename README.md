...
- Accueil guidé (étapes + formulaire de création dans la zone principale) lorsque l’utilisateur n’a aucun projet, pour les petits écrans et la barre latérale repliée (`src/ui_components.py`, `src/empty_project_onboarding.py`, `src/project_session.py`). Règle produit **sidebar = contexte** / **onglet Projets = actions** : voir [`docs/curator_navigation_product_rule.md`](docs/curator_navigation_product_rule.md) (issue-011).
