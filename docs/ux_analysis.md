...
**Comportement actuel** : plus de `st.form` sur cet onglet ; brouillon et texte généré utilisent des `text_area` avec des clés `session_state` **scopées projet + utilisateur** (préfixe `new_entry_{project_id}_u_{user_id_sanitisé}_*`, détail dans [`docs/session_state_keys.md`](session_state_keys.md)). Les boutons « Générer texte » / « Générer brouillon » utilisent `st.button(..., on_click=…)` pour appliquer le résultat LLM **avant** l’instanciation des widgets dans le run (exigence Streamlit : pas d’écriture sur une clé de widget après son `text_area` dans le même script). « Enregistrer » relit `session_state` au clic ; après succès, un drapeau `_pending_clear_new_entry_{project_id}_u_{user_id_sanitisé}` déclenche le vidage des buffers **au run suivant**, avant les `text_area`, pour la même raison. Les anciennes clés `new_generated_*` sont ignorées puis supprimées à l’entrée de l’onglet ; les clés legacy sans segment utilisateur ne sont plus recopiées vers le compte courant (voir la section « Migration » dans `docs/session_state_keys.md`).
...
**Localisation** : `main.py` appelle une seule fois `render_post_save_stylometric_feedback_banner` (avant `st.tabs`) pour éviter qu’un onglet ne consomme le payload avant un autre ; `src/ui_components.py` — `_store_post_save_stylometric_feedback` / `_render_post_save_stylometric_feedback`, déclencheurs dans `render_tab_edition` et `render_tab_ajout` ; libellés et phrase de fraîcheur dans `src/post_save_feedback_display.py` ; `src/nlp_engine.py` — `compute_row_cache`, `row_nlp_feedback_bundle_after_persist`, `curator_advices_after_save`.

Après une sauvegarde réussie (`update_project_entries` sans erreur), l’application invalide le cache Streamlit des entrées, relit le projet via `cached_load_project_entries` (wrapper `@st.cache_data` autour de `load_project_entries`, voir issue S4 / `src/project_entries_cache.py`), construit le bundle avec `row_nlp_feedback_bundle_after_persist` (métriques et cache alignés sur la ligne persistée) puis enregistre l’état d’affichage en `st.session_state` avant `st.rerun()`. Au run suivant, un bloc **« Retour stylistique (ligne enregistrée) »** (au-dessus des onglets) affiche le score de cohérence, le TTR, le contraste syntaxique, un libellé qualitatif (`qualitative_coherence_feedback`, aligné sur `coherence_level`) et jusqu’à trois conseils issus de `prioritized_actions` / `curator_advices_after_save`. En cas d’échec de persistance, aucun feedback n’est stocké (pas de faux positif curateur).
...
| N3 | **Fait (issue-034)** : libellés techniques du bloc IA en français métier + `help=` (variables `LLM_*` inchangées côté persistance) | Accessibilité non-technique |
