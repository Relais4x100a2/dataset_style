...
Après **création projet**, **suppression projet**, **sauvegarde nouvelle entrée**, **sauvegarde édition**, et **suppression de compte côté Super Admin** (flux SA-DELETE), le code applicatif invalide le cache Streamlit des entrées avant `st.rerun()`. Toute couche API devra documenter l’équivalent (version de ressource, `Cache-Control`, ou réponse obligeant le client à refetch les `entries`). **Webapp** : voir corps `entries` sur `POST`/`PATCH` entrées (`src/webapp/app.py`) + `GET .../entries` avec paramètres `edition_*` pour les filtres liste (équivalent onglet Gestion & édition).
