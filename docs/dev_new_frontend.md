...
- `PATCH /api/projects/{project_id}/entries/{entry_id}` — champs partiels (`EntryPatchBody`) ; persistance `entry_mutations.apply_entry_field_updates` → `persist_edited_entry_with_nlp_cache` → `update_project_entries`.
